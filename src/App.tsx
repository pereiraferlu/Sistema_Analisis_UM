...
export default function App() {
  const [sucursalFiles, setSucursalFiles] = useState<{name: string, data: LogisticsData[], totals?: { piezas: number; bultos: number }, presupuestos?: Record<string, number>, missingColumns?: string[]}[]>([]);
  const [consultaGlobalFiles, setConsultaGlobalFiles] = useState<{name: string, data: ConsultaGlobalData[]}[]>([]);
  const [hdrDistribuidorFiles, setHdrDistribuidorFiles] = useState<{name: string, data: DistribuidorData[]}[]>([]);
  const [historialFiles, setHistorialFiles] = useState<{name: string, data: LogisticsData[]}[]>([]);
  ...
  const pendingData = sucursalFiles.flatMap(f => f.data);
  ...
  const totals = sucursalFiles.reduce((acc, f) => ({
    piezas: acc.piezas + (f.totals?.piezas || 0),
    bultos: acc.bultos + (f.totals?.bultos || 0)
  }), { piezas: 0, bultos: 0 });
  const pendingPresupuestos = sucursalFiles.reduce((acc, f) => ({ ...acc, ...(f.presupuestos || {}) }), {});
  const missingColumns: string[] = Array.from(new Set(sucursalFiles.flatMap(f => f.missingColumns || [])));
  const fileName = sucursalFiles.map(f => f.name).join(", ");
  ...
  const handleDataLoaded = (
    ...
    if (type === "SUCURSAL") {
      setSucursalFiles(prev => {
        const newFiles = [...prev, { name, data: parsedData as LogisticsData[], totals: parsedTotals, presupuestos: presupuestosMap, missingColumns: missingCols }];
        setLastRawData(newFiles.flatMap(f => f.data));
        setLastRawTotals(newFiles.reduce((acc, f) => ({
          piezas: acc.piezas + (f.totals?.piezas || 0),
          bultos: acc.bultos + (f.totals?.bultos || 0)
        }), { piezas: 0, bultos: 0 }));
        setLastRawPresupuestos(newFiles.reduce((acc, f) => ({ ...acc, ...(f.presupuestos || {}) }), {}));
        return newFiles;
      });
    } else if (type === "CONSULTA_GLOBAL") {
      setConsultaGlobalFiles(prev => [...prev, { name, data: parsedData as ConsultaGlobalData[] }]);
    } else if (type === "HDR_DISTRIBUIDOR") {
      setHdrDistribuidorFiles(prev => [...prev, { name, data: parsedData as DistribuidorData[] }]);
    } else if (type === "HISTORIAL") {
      setHistorialFiles(prev => [...prev, { name, data: parsedData as LogisticsData[] }]);
    }
  };

  const removeFile = (index: number, type: "SUCURSAL" | "CONSULTA_GLOBAL" | "HDR_DISTRIBUIDOR" | "HISTORIAL") => {
    if (type === "SUCURSAL") {
      setSucursalFiles(prev => {
  ...
  const handleConfirm = (
    ...
    const finalData = [...data.filter(d => !(idsToRemove || []).includes(getRouteId(d))), ...updatedData];
    setData(finalData);
    ...
    setSucursalFiles([]); // Clear pending files after confirmation
  };
  ...
  const performCrossAnalysis = (
    ...
    sucursales.forEach(sucName => {
      ...
      sucRows.forEach(row => {
        const key = `${row.distribuidor}_${row.fecha}`;
      ...
      Object.entries(groups).forEach(([key, rows]) => {
        ...
        const e_db = relevantGlobal.filter(g => g.estado.toLowerCase().includes("entregado")).length;
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full p-8 relative">
              ...
              <FileUpload 
                ...
                description="Archivo principal de costos y rutas (.xls, .xlsx)"
              />
              ...
              {sucursalFiles.length > 0 && (
                <div className="mt-8 flex justify-center">
                  ...
                  >
                    ...
                    <span>Analizar Nuevos Archivos</span>
                  </button>
                </div>
              )}
            </div>
    ...
    <div className="min-h-screen bg-secondary-50 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl text-center mb-8 relative z-10">
        ...
        <p className="mt-4 text-lg text-secondary-600 max-w-2xl mx-auto">
          ...
          Opcionalmente, cargue la consulta global y HDR distribuidores para un cruce de datos completo.
        </p>
      </div>

      <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative z-10 mb-8">
        ...
        <div className="space-y-4">
          <FileUpload 
            ...
            description="Datos del sistema para cruce (.csv, .xlsx)"
          />
          <div className="space-y-2">
        ...
        <div className="space-y-4">
          <FileUpload 
            ...
            description="Datos de distribuidores para cruce (.csv, .xlsx)"
          />
          <div className="space-y-2">
        ...
        <div className="space-y-4">
          <FileUpload 
            ...
            description="Historial de meses pasados (.xls, .xlsx)"
          />
          <div className="space-y-2">
